...
function NotificationPage() {
  ...
  // a state to store the notifications
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
        try {
            ...
            const unreadId = data.filter(notification => !notification.read).map(notification => notification.id)
            if (unreadId.length > 0) await markNotificationsAsRead(unreadId);
        } catch (error) {
            toast({
                  ...
                  ? (
                    ...
                    notifications.map((notification) => (
                      <div
                        key={notification.id}
                        className={`
                          flex items-start gap-4 p-4 border-b hover:bg-muted/25 transition-colors 
